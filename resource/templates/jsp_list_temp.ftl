...
<script type="text/javascript">
	...
	function save() {
		...
		${dd}('#fm').form('submit', {
			onSubmit : function() {
				var rr=${dd}(this).form('enableValidation').form('validate');
				if (rr) {
			...
			success : function(result) {
				...
				layer.close(index);
				if (result.success) {
					${dd}('#dlg').dialog('close'); // close the dialog
					${dd}('#dg').datagrid('reload'); // reload the user data
				} else {
					${dd}.messager.show({
						title : '提示',
						msg : result.msg
...
<body>
	...
	<div id="content" region="center" title="列表" style="padding:5px;">
		<table id="dg" class="easyui-datagrid"
			style="width:98%;min-height:400px" url="${package}/${table.classNameL}AjaxPage.do"
			iconCls="icon-save" rownumbers="true" pagination="true"
			singleSelect="true" toolbar="#toolbar">
			<thead>
				<tr>
					<#list table.cloums as c>
					<th field="${c.cloumName}" align="center" width="200"><#if c.cnName??>${c.cnName}</#if></th>
					</#list>
				</tr>
			</thead>
		</table>
		<div id="toolbar">
			...
			<div style="margin-bottom:5px">
				<a href="javascript:void(0)" class="easyui-linkbutton"
					...
					onclick="doDelete('${package}/${table.classNameL}AjaxDelete.do')">删除</a>
			</div>
		</div>
		<div id="dlg" class="easyui-dialog"
			...
			<form id="fm" name="fm" method="post" action="${package}/${table.classNameL}AjaxSave.do"
				data-options="novalidate:true">
				<input type="hidden" id="id" name="id">
				<#list table.cloums as c>
				<div class="fitem">
					<label><#if c.cnName??>${c.cnName}</#if>:</label> <input id="${c.cloumName}" name="${c.cloumName}"
		...
		<div id="dlg-buttons">
			<a href="javascript:void(0)" class="easyui-linkbutton c6"
				...
				href="javascript:void(0)" class="easyui-linkbutton"
				iconCls="icon-cancel" onclick="javascript:${dd}('#dlg').dialog('close')"
